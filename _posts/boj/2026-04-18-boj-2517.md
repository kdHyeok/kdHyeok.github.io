...
<div class="post-tags-top"><a href="/tags/자료-구조/" class="post-tag">자료 구조</a> <a href="/tags/세그먼트트리/" class="post-tag">세그먼트트리</a> <a href="/tags/값-좌표-압축/" class="post-tag">값 / 좌표 압축</a></div>
...
- **목표:** 현재 달리는 순서대로 나열된 선수들이 자신보다 앞에 있는 선수 중 실력이 낮은 사람들을 모두 추월했을 때 얻을 수 있는 최선의 등수를 구한다.

- **입력 상태:** 선수의 수 N(3 ≤ N ≤ 500,000)과 각 선수의 평소 실력(1 ≤ 실력 ≤ 1,000,000,000)이 달리는 순서대로 주어진다.

- **핵심 조건:** 선수는 자신보다 앞에 있으면서 실력이 낮은 사람만 추월할 수 있으므로, '최선의 등수 = 현재 순번 - 내 앞의 선수 중 나보다 실력이 낮은 사람의 수'로 계산한다.
...
- **시간 복잡도:** O(N log N) — 좌표 압축을 위한 정렬과 각 선수별 세그먼트 트리 업데이트 및 쿼리 비용의 합이다.

### 핵심 아이디어

실력 수치가 최대 10억이므로 좌표 압축을 통해 1~N 사이의 인덱스로 변환한 뒤, 세그먼트 트리를 이용해 현재 선수보다 앞에 지나간 선수들의 실력 분포를 관리한다. 각 선수를 순서대로 처리하며 트리에서 자신보다 낮은 실력 구간의 합을 구해 추월 가능한 인원수를 계산한다.

### ① 좌표 압축 (Coordinate Compression)

실력 데이터의 범위(10억)가 메모리 제한 내의 배열 인덱스로 쓰기에 너무 크기 때문에, 상대적인 순위 정보(1~N)로 변환하여 세그먼트 트리의 인덱스로 활용하기 위함이다.
...
현재 선수의 실력 순위(rank_idx)를 기준으로 1부터 rank_idx-1까지의 구간 합을 구한다. 이 결과는 내 앞에 달리고 있는 선수들 중 나보다 실력이 낮아 내가 앞지를 수 있는 사람의 총 합이며, 이를 현재 등수(i+1)에서 빼서 최선의 등수를 도출한다.
...
현재 선수의 계산이 끝난 후, 자신의 실력 위치에 해당하는 트리 노드에 1을 더한다. 이는 다음에 처리될(내 뒤에서 달리는) 선수들이 '자신보다 앞에 있는 선수들의 실력 분포'를 쿼리할 때 현재 선수의 정보를 포함할 수 있게 하기 위함이다.
